...
local function factory(args)
    ...
    function mem.hide()
        if not mem.notification then return end
  ...
  function mem.update()
    ...
    local notifytable = { [1]   = string.format("%-5s\t%-5s\t%-5s\t%-5s\t%-4s\n", "type", "total", "used", "free", "unit") }
    notifytable[#notifytable+1] = string.format("%-5s\t%-5d\t%-5d\t%-5d\t%-4s\n", "ram", mem_now.total, mem_now.used, mem_now.free,  "MB")
    notifytable[#notifytable+1] = string.format("%-5s\t%-5d\t%-5d\t\t%-5d\t%-4s", "swap", mem_now.swap, mem_now.swapused, mem_now.swapf, "MB")
    mem.notification_preset.text = tconcat(notifytable) 
  end
